...
def split_ill(params: List[SceneParameters],
              num_samples: int = None
              ) -> Tuple[List[SceneParameters],
                         List[SceneParameters]]:
    """Returns a tuple of SceneParameters split by their ill property.

    Attrs:
        params: List of SceneParfameters to split ill state
        num_samples: exact number of SceneParameters to select per class. None means all availabel.


    """
    return ([p for p in params if p.ill == 0][:num_samples],
            [p for p in params if p.ill == 1][:num_samples])